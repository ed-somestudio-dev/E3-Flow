    ...
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Defina e acompanhe seus limites mensais</p>
        </div>
          ...
          return (
            <motion.div key={budget.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex justify-between text-sm mb-2">
                ...
                <span className="text-muted-foreground">de {fmt(budget.amount)}</span>
              </div>
              <Progress value={pct} className="h-2" />
              {over && <p className="text-xs text-destructive mt-2">Acima do orçamento em {fmt(spent - budget.amount)}</p>}
              {nearLimit && <p className="text-xs text-warning mt-2">Próximo do limite ({Math.round(pct)}% usado)</p>}
            </motion.div>
          );
        })}
