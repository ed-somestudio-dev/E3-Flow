...
export function AuthProvider({ children }: { children: React.ReactNode }) {
        ...
        if (!finalSession) {
          ...
          if (offlineDataStr && !assertOnline()) {
             try {
               ...
               const isWithin30Days = true; // Controle de bloqueio delegado ao subscription-context
               if (isWithin30Days) {
                 console.log('[AuthContext] Usando fallback offline para o usuário:', offlineData.user.email);
                 finalSession = {
  ...
  const signOut = async () => {
    localStorage.setItem('e3flow_intentional_logout', 'true');
    localStorage.removeItem('e3flow_guest_id');
    localStorage.removeItem('e3flow_offline_user');
    await supabase.auth.signOut();
    setSession(null);
    setTimeout(() => {
      localStorage.removeItem('e3flow_intentional_logout');
    }, 1000);
  };
